...
- 100종은 손으로 나열한 게 아니라 **4개 축의 전수 조합**입니다:
  목표 5(스프린트/30조각 수거전/서바이벌/편식가/콤보 러시) × 조작 2(유영/대시)
  × 위험 2(청정/위성 회피) × 테마 5(민트/핑크/라벤더/별빛/오로라) = 100.
  별점 로그에 축 정보가 함께 남아 "어떤 축의 어떤 값이 인기인가"로 해석할 수 있습니다.
- `/arcade` 허브(목표별 그룹 그리드) → `/arcade/g001`~`g100` (빌드 시 전부 정적 생성).
- 게임 규칙은 `src/lib/arcade/engine.ts`의 **순수 로직**(React/캔버스 없음, now 주입)이고,
  `GameCanvas.tsx`는 rAF 루프·드로잉·입력만 담당합니다.
- 엔진 검증: `npx -y tsx scripts/arcade-sim-test.ts` — 가상 시계로 5개 목표
  메커니즘·이동 모델·변형 무결성을 헤드리스로 시뮬레이션합니다.
- ⚠️ **id는 조합 순서에서 나옵니다.** `variants.ts`의 축 순서·값을 중간에 바꾸면
  기존 별점/기록의 id가 어긋나므로 변형 추가는 배열 끝에만.
